...
it("wires authenticated backend transport and legacy requests with loopback defaults", async () => {
  ...
  // The actual configured transport reaches this test listener; /api/info is intentionally absent.
  await expect(mocks.clients[0]?.server.info()).rejects.toBeDefined();
  mocks.unavailable[0]?.();
  await cleanup();
});
