...
const mocks = vi.hoisted(() => ({
  ...
  stopped: vi.fn(),
  started: vi.fn(),
  checks: [] as Array<(url: string, timeout: number) => Promise<boolean>>,
...
vi.mock("./health.js", () => ({
  ...
  HealthMonitor: class {
    constructor(
      ...
      _config: object,
      unhealthy: () => void,
      check: (url: string, timeout: number) => Promise<boolean>,
    ) {
...
it("boots the HTTP listener and releases signals and resources on shutdown", async () => {
  ...
  expect(await mocks.checks[0]?.("http://host", 100)).toBe(true);
  mocks.unhealthy[0]?.();
  await cleanup();
...
it("validates bootstrap input and checks whether the owning process still exists", async () => {
  await expect(bootstrap({})).rejects.toThrow();
  const values = env(await port());
  values.OPENCODE_RUN_SERVER_PARENT_PID = "99999999";
  const cleanup = await bootstrap(values);
  if (cleanup !== undefined) cleanups.push(cleanup);
  expect(await mocks.checks[0]?.("http://host", 100)).toBe(false);
});
